...
class QuestionTest < ActiveSupport::TestCase
  ...
  context "Creating answers" do 
    setup do
      @creator = Factory.create(:learner)
      @event = Factory.create(:event, creator: @creator, last_modifier: @creator)
    end
    
    context "for a boolean question" do
      setup do
        @q_boolean = Factory.create(:question, responsetype: Question::BOOLEAN, responses: ['y','n'], creator: @creator, event: @event)
      end
    
      should "create one answer if doesn't exist" do
        assert_equal(0,@q_boolean.answers.count)
        @q_boolean.create_or_update_answers(creator: @creator, update_value: 0)
        assert_equal(1,@q_boolean.answers.count)
      end
      
      should "update answer if one exists" do
        Answer.create(creator: @creator, question: @q_boolean, value: 0)
        assert_equal(1,@q_boolean.answers.count)
        @q_boolean.create_or_update_answers(creator: @creator, update_value: 1)
      ...
      should "return the expected answer data given a set of answers" do
        ...
        @q_boolean.responses.each do |response| 
          Answer.create(creator: @creator, question: @q_boolean, value: 1, response: response)
    ...
    context "for a scale question" do
      setup do
        @q_scale = Factory.create(:question, responsetype: Question::SCALE, responses: ['least','most'], range_start: 1, range_end: 5, creator: @creator, event: @event)
      end
    
      should "create one answer if doesn't exist" do
        assert_equal(0,@q_scale.answers.count)
        @q_scale.create_or_update_answers(creator: @creator, update_value: 42)
        assert_equal(1,@q_scale.answers.count)
      end
      
      should "update answer if one exists" do
        Answer.create(creator: @creator, question: @q_scale, value: 42)
        assert_equal(1,@q_scale.answers.count)
        @q_scale.create_or_update_answers(creator: @creator, update_value: 24)
      ...
      should "return the expected answer data given a set of answers" do
        ...
        @q_scale.range_start.upto(@q_scale.range_end).each do |value| 
          Answer.create(creator: @creator, question: @q_scale, value: value)
    ...
    context "for a multivote boolean question" do
      setup do
        @q_multivote_boolean = Factory.create(:question, responsetype: Question::MULTIVOTE_BOOLEAN, responses: ['uno','dos','tres','quatro'], creator: @creator, event: @event)
      end

      should "create answers for the provided values if they don't exist" do
        expected_answers = 3
        answer_values = ['uno','dos','quatro','nonsense']
        @q_multivote_boolean.create_or_update_answers(creator: @creator, update_value: answer_values)
      ...
      should "update answers if exists" do
        ...
        initial_responses.each do |response|  
          Answer.create(creator: @creator, question: @q_multivote_boolean, value: 1, response: response)
        end
        ...
        @q_multivote_boolean.create_or_update_answers(creator: @creator, update_value: update_responses)
      ...
      should "delete all answers if nil update_value provided" do
        ...
        @q_multivote_boolean.create_or_update_answers(creator: @creator, update_value: nil)
